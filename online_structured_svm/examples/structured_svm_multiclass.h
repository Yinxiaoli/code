...
class MulticlassStructuredSVM : public StructuredSVM {
 public:
  ...
  double Inference(StructuredData *x, StructuredLabel *ybar, SparseVector *w, 
		   StructuredLabel *y_partial=NULL, StructuredLabel *y_gt=NULL, double w_scale=1);
  double Loss(StructuredLabel *y_gt, StructuredLabel *y_pred);
  double ImportanceSample(StructuredData *x, SparseVector *w, StructuredLabel *y_gt, 
			  struct _SVM_cached_sample_set *set, double w_scale=1);
 ...
 public:
  ...
  void SetClassConfusionCosts(double **c);
 ...
 private:
  ...
  SVM_cached_sample **cached_samples;
  int *cached_num_samples;
  ...
  void CreateSamples(struct _SVM_cached_sample_set *set, MulticlassStructuredData *x, MulticlassStructuredLabel *y_gt);
  int LoadExample(MulticlassStructuredData *x, MulticlassStructuredLabel *y, bool lazyLoad, char* &line);
};
...
class MulticlassStructuredData : public StructuredData {
  ...
  long long seek_pos; /**< seek location in fin, for lazily loading psi */
  FILE **fins;
  bool readBinary;
...
class MulticlassStructuredDataset : public StructuredDataset {
 public:
  FILE **fins;
  int numThreads;
 MulticlassStructuredDataset() : StructuredDataset() { fins = NULL; }
  ~MulticlassStructuredDataset() { 
    if(fins)
