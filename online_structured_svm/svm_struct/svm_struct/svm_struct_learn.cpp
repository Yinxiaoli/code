		      ...
		      STRUCTMODEL *sm, int alg_type)
{
  ...
  if(USE_FYCACHE) {
    ...
    for(i=0;i<n;i++) {
      fprintf(stderr, "Precomputing features %d...\n", i);
      fy=psi(ex[i].x,ex[i].y,sm,sparm);
      finished_example_iteration(ex[i].x,ex[i].y,sparm);
      if(kparm->kernel_type == LINEAR) {
  ...
  do { /* iteratively increase precision */
    ...
    cont = finalize_iteration(ceps,0,sample,sm,cset,alpha,sparm);
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
      ...
      cont = finalize_iteration(ceps,cached_constraint,sample,sm,cset,alpha,sparm);
  } while(cached_constraint || (ceps > sparm->epsilon) || cont);
	...
	to the most violated constraint for example ex */
{
  ...
  finished_example_iteration(ex->x, ex->y, sparm);
}
