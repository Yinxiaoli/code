...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(!sparm->learner) {
    sparm->learner = g_learner;
    sparm->learner->SetC(sparm->C);
    sparm->learner->SetEpsilon(sparm->epsilon);
  }

  StructuredDataset *dataset = strlen(file) ? sparm->learner->LoadDataset(file) : sparm->learner->GetTrainset();
  sample.n = dataset->num_examples;
  sample.examples=(EXAMPLE *)my_malloc(sizeof(EXAMPLE)*sample.n);
  for(int i = 0; i < sample.n; i++) {
    sample.examples[i].x.data = dataset->examples[i]->x;
    sample.examples[i].y.data = dataset->examples[i]->y;
  }
  sparm->learner->SetTrainset(dataset);
  sparm->learner->SetSumWScale(1);
			      ...
			      KERNEL_PARM *kparm)
{
  ...
  sm->sizePsi=sparm->learner->GetSizePsi(); /* replace by appropriate number of features */
}
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  LABEL y;
  y.data = sparm->learner->NewStructuredLabel(x.data);

  SparseVector *w = new SparseVector(sm->svm_model->lin_weights+1, sparm->learner->GetSizePsi());
  sparm->learner->Inference(x.data, y.data, w);
  delete w;
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  /* Finds the label ybar for pattern x that that is responsible for
     ...
     empty_label(y). */
  LABEL ybar;
  ybar.data = sparm->learner->NewStructuredLabel(x.data);

  SparseVector *w = new SparseVector(sm->svm_model->lin_weights+1, sparm->learner->GetSizePsi());
  sparm->learner->Inference(x.data, ybar.data, w, NULL, y.data);
  delete w;
		 ...
		 STRUCT_LEARN_PARM *sparm)
{
  ...
  int num;
  SparseVector psi = sparm->learner->Psi(x.data,y.data);
  SparseVectorElement *elements = psi.get_elements(&num);
  WORD *words = new WORD[num+1];
  for(int i = 0; i < num; i++) {
    words[i].wnum = elements[i].ind+1;
    words[i].weight = elements[i].val;
  }
  words[num].wnum = 0;  words[num].weight = 0;  
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  return sparm->learner->Loss(y.data, ybar.data);

  return 0;
}

void finished_example_iteration(PATTERN x, LABEL y, STRUCT_LEARN_PARM *sparm) {
  StructuredExample ex;
  ex.x = x.data;
  ex.y = y.data;
  sparm->learner->OnFinishedIteration(&ex);
  ex.x = NULL;
  ex.y = NULL;
}
			       ...
			       STRUCT_LEARN_PARM *sparm)
{
  ...
  sparm->learner->SetWeights(sm->svm_model->lin_weights+1);
  if(sparm->modelfile)
    sparm->learner->DumpModelIfNecessary(sparm->modelfile);
  return(0);
}
...
STRUCTMODEL read_struct_model(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(!sparm->learner) 
    sparm->learner = g_learner;
  sparm->learner->Load(file);

  int sizePsi = sparm->learner->GetSizePsi();
  MODEL *model = (MODEL *)my_malloc(sizeof(MODEL));
  memset(model, 0, sizeof(MODEL));
  model->lin_weights=(double*)my_malloc(sizeof(double)*(sizePsi+1));
  SparseVector *w = sparm->learner->GetCurrentWeights();
  w->get_non_sparse<double>(sizePsi, model->lin_weights+1);
  delete w;
...
void        write_label(FILE *fp, LABEL y, STRUCT_LEARN_PARM *sparm)
{
  /* Writes label y to file handle fp. */
  char str[10000];
  Json::FastWriter writer;
  Json::Value v = y.data->save(sparm->learner);
