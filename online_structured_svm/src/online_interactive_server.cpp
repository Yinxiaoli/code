...
bool StructuredLearnerRpc::RemoveExample(const Json::Value& root, Json::Value& response) {
  if(learner) {
    response["success"] = learner->RemoveExample(root.get("index", -1).asInt());
    return true;
  } else
    return false;
}
...
bool StructuredLearnerRpc::RelabelExample(const Json::Value& root, Json::Value& response) {
  if(learner) {
    response["success"] = learner->RelabelExample(root.get("index", -1).asInt());
    return true;
  } else
    return false;
}
...
void StructuredLearnerRpc::EvictSession(int ind, Session *replace) {
  Lock();
  if(ind == -1) {
    unsigned long b = timestamp+1;
    unsigned long b2 = timestamp+1;
    int ind2 = -1;
    for(int i = 0; i < num_sessions; i++) {
	...
	ind = i;
      ...
      if(sessions[i].timestamp < b2 && sessions[i].finished) {
        b2 = sessions[i].timestamp;
        ind2 = i;
      }
      if(ind2 >= 0) ind = ind2;
    }
  ...
  while(!omp_test_lock(&sessions[ind].lock))
    ;
  ...
  //fprintf(stderr, "Done evict %d\n", ind);
  if(replace) {
    sessions[ind] = *replace;
    //fprintf(stderr, "Create session after evict %d\n", ind);
  }
/*
...
bool StructuredLearnerRpc::NewSession(const Json::Value& root, Json::Value& response) {
  ...
  if(root.get("mkdir", false).asBool()) 
     CreateDirectoryIfNecessary(sessDir2, 0777);
  ...
  s.timestamp_start = time(NULL);

  omp_init_lock(&s.lock);
  if(num_sessions >= MAX_SESSIONS) 
    EvictSession(-1, &s);
  else {
    //fprintf(stderr, "Create session %d\n", num_sessions);
...
bool StructuredLearnerRpc::SaveCurrentModel(const Json::Value& root, Json::Value& response) {
  ...
  if(learner) {
    ...
    if(strlen(cache_name)) {
      learner->Lock();
      learner->GetChooser()->SaveCachedExamples(cache_name, root.get("saveCacheFull", false).asBool());
      learner->Unlock();
    }
    return true;
  } else
...
bool StructuredLearnerRpc::GetStatistics(const Json::Value& root, Json::Value& response) {
  if(learner) {
    ...
    double x, y;
    long t;
    StructuredSVMStatistics *stats = learner->GetStatistics();
	...
	stats->GetStatisticsByExample(window, &t, &gen_err_buff, &emp_err_buff, &model_err_buff, &reg_err_buff, &loss_buff);
	plotter.SetXLabel("Number of Training Examples (n)");
      } else {
	if(!strcmp(plot_by, "time")) {
	  plotter.SetXLabel("Training Time (sec)");
	  stats->GetStatisticsByIteration(window, &t, &gen_err_buff, &emp_err_buff, &model_err_buff, &reg_err_buff, &loss_buff, &time_buff);
	} else {
	  plotter.SetXLabel("Training Iterations");
	  stats->GetStatisticsByIteration(window, &t, &gen_err_buff, &emp_err_buff, &model_err_buff, &reg_err_buff, &loss_buff, NULL);
	}
      }
      plotter.AddPlot(time_buff, gen_err_buff, t, 0, 1, "Generalization Error", "class1");
      plotter.AddPlot(time_buff, emp_err_buff, t, 0, 1, "Training Error", "class2");
...
bool StructuredLearnerRpc::GetTrainset(const Json::Value& root, Json::Value& response) {
  char cmp[1000];
  
  // visualize can be "ave_slack", "slack", "alpha"
  strcpy(cmp, root.get("sortBy", "").asString().c_str());
  int maxExamples = root.get("maxExamples", 100).asInt();
  int startFrom = root.get("startFrom", 0).asInt();
  bool getSuggestedLabel = root.get("getSuggestedLabel", false).asBool();
  bool getX = root.get("getX", true).asBool();
  bool getY = root.get("getY", true).asBool();
  if(learner && learner->GetTrainset()) {
    StructuredDataset d;
    ...
    if(!strcmp(cmp, "slack")) qsort(d.examples, d.num_examples, sizeof(StructuredExample*), example_slack_cmp);
    else if(!strcmp(cmp, "ave_slack")) qsort(d.examples, d.num_examples, sizeof(StructuredExample*), example_ave_slack_cmp);
    else if(!strcmp(cmp, "alpha")) qsort(d.examples, d.num_examples, sizeof(StructuredExample*), example_alpha_cmp);
    else if(!strcmp(cmp, "suggest")) qsort(d.examples, d.num_examples, sizeof(StructuredExample*), example_suggest_cmp);
    Json::Value ex;
    for(int i = startFrom; i < d.num_examples && i < startFrom+maxExamples; i++) {
      Json::Value o;
      o["index"] = learner->GetTrainsetId(i);
      if(getX) o["x"] = d.examples[i]->x->save(learner);
      if(getY) o["y"] = d.examples[i]->y->save(learner);
      if(getSuggestedLabel && d.examples[i]->set && d.examples[i]->set->ybar)
	o["suggestedLabel"] = d.examples[i]->set->ybar->save(learner);
      ex[i-startFrom] = o;
    }
    response["examples"] = ex;
...
void StructuredLearnerRpc::parse_command_line_arguments(int argc, const char **argv) {
  ...
  while(i < argc) {
    if(!strcmp(argv[i], "-P")) { 
      ...
      assert(i+3 <= argc);
      params->dumpModelStartTime = atof(argv[i+1]);
      params->dumpModelFactor = atof(argv[i+2]);
      i += 3;
    } else if(!strcmp(argv[i], "-W")) {
      // Window for estimating convergence statistics
      assert(i+2 <= argc);
      learner->GetStatistics()->SetWindow(atoi(argv[i+1]));
      i += 2;
      ...
      assert(i+2 <= argc);
      params->eps = atof(argv[i+1]);
      i += 2;
    } else if(!strcmp(argv[i], "-m")) {
      // optimization method
      assert(i+2 <= argc);
      params->method = OptimizationMethodFromString(argv[i+1]);
      i += 2;
    } else if(!strcmp(argv[i], "-D")) {
      // Determines how much stuff to keep in memory 
      assert(i+2 <= argc);
      params->memoryMode = MemoryModeFromString(argv[i+1]);
      i += 2;
    } else if(!strcmp(argv[i], "-T")) {
      // Number of threads
      assert(i+2 <= argc);
      params->runMultiThreaded = atoi(argv[i+1]);
      i += 2;
      ...
      params->randomize = atoi(argv[i+1]) > 0;
      i += 2;
    } else if(!strcmp(argv[i], "-v")) {
      params->debugLevel = atoi(argv[i+1]);
      i += 2;
    } else if(!strcmp(argv[i], "-V")) {
      learner->SetValidationFile(argv[i+1]);
      i += 2;
    } else if(!strcmp(argv[i], "-M")) {
      params->max_samples = atoi(argv[i+1]);
      i += 2;
    } else if(argv[i][0] == '-') {
      i += 2;
    } else
...
int StructuredLearnerRpc::main(int argc, const  char **argv) {
  ...
  if(strlen(trainfile)) {
    ...
    if(learner->GetTrainParams()->randomize) learner->GetTrainset()->Randomize();
...
void StructuredLearnerRpc::AddMethods() {
  ...
  if(train) {
    ...
    Json::Value get_trainset_parameters, get_trainset_returns;
    get_trainset_parameters["sortBy"] = "If specified, sort training examples by some criteria.  Options are 'none' (visualize the whole training set), 'slack' (order examples based on last slack value), 'ave_slack' (order examples based on average slack during training), 'alpha' (order examples by dual parameter weight)";
    get_trainset_parameters["maxExamples"] = "The maximum number of training examples to visualize";
    get_trainset_parameters["startFrom"] = "Get examples starting from a particular index";
    get_trainset_parameters["getX"] = "For each example, get a JSON encoding of the data 'x'";
    get_trainset_parameters["getY"] = "For each example, get a JSON encoding of the label 'y'";
    get_trainset_parameters["getSuggestedLabel"] = "For each example, get a JSON encoding of a suggested relabeling of 'y'";
    get_trainset_returns["examples"] = "An array of examples, each of which will have fields index, x, y, suggestedLabel";
  server->RegisterMethod(new JsonRpcMethod<StructuredLearnerRpc>(this, &StructuredLearnerRpc::GetTrainset, "get_trainset", "Get JSON encoding of the current training set", get_trainset_parameters, get_trainset_returns));
